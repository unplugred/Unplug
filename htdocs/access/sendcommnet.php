<html lang="en"><head><style>body{background-color:black;}</style><?php
	if(isset($_POST['submit'])){
		$comment = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", htmlentities(substr($_POST['comment'], 0, 4000)));
		if(!empty($comment) && $comment != "\n"){
			...
			fclose($handle);
			?><script>sessionStorage.setItem("PageVisited","True");window.history.back();</script><?php
		}
	}
